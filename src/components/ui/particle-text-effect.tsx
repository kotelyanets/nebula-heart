...
const DEFAULT_WORDS = ["HELLO", "World", "Particle", "Text"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
    ...
    const nextWord = (word: string, canvas: HTMLCanvasElement) => {
        ...
        for (let i = particleIndex; i < particles.length; i++) {
            // Fix: Pass generateRandomPos to kill method or modify class logic. 
            // Modifying class method above to accept it.
    ...
    const animate = () => {
        ...
        // Background with motion blur - Clear to transparent for overlay usage
        ctx.clearRect(0, 0, canvas.width, canvas.height)
        // Optional: Keep motion blur if desired, but transparent bg is safer for overlay
        ...
        if (mouseRef.current.isPressed && mouseRef.current.isRightClick) {
            particles.forEach((particle) => {
                ...
                if (distance < 50) {
                    particle.kill(canvas.width, canvas.height, generateRandomPos)
                }
            })
        }
    ...
    return (
        <div className="w-full h-80 flex items-center justify-center overflow-hidden bg-transparent">
            <canvas
                ref={canvasRef}
